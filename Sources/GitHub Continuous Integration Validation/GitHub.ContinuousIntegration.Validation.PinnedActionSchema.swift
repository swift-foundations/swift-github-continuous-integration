...
extension GitHub.ContinuousIntegration.Validation {
    ...
    public struct PinnedActionSchema: Validator {
        ...
        static func findings(in document: GitHub.ContinuousIntegration.Workflow.Document, subject: Subject) -> [Finding] {
        ...
        static func undeclaredInputFindings(
            document: GitHub.ContinuousIntegration.Workflow.Document, job: GitHub.ContinuousIntegration.Workflow.Job, uses: String,
            with: GitHub.ContinuousIntegration.Workflow.YAML.Mapping?, schema: PinnedContent.Schema,
            repository: String
        ) -> [Finding] {
        ...
        static func absentOutputFindings(
            document: GitHub.ContinuousIntegration.Workflow.Document, schemaByStepID: [String: PinnedContent.Schema],
            repository: String
        ) -> [Finding] {
            var texts: [String] = []
        ...
        /// style `GitHub.ContinuousIntegration.Validation.SchemaCorrespondence` already uses for
        /// `.settings.<key>`.
        ...
        static func collectScalarText(_ node: GitHub.ContinuousIntegration.Workflow.YAML.Node, into texts: inout [String]) {
            switch node {
            case .text(let value):
                texts.append(value)
