...
extension GitHub.ContinuousIntegration.Validation {
    ...
    public enum PinnedContent {
        ...
        /// the same bounded YAML reader `GitHub.ContinuousIntegration.Workflow.Document` uses.
        ///
        ...
        static func parsed(_ text: String, object: String) -> Resolution {
            guard let root = try? GitHub.ContinuousIntegration.Workflow.YAML.Parser.parse(text) else {
        ...
        static func keys(of node: GitHub.ContinuousIntegration.Workflow.YAML.Node?) -> Set<String> {
